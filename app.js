...
function getWeeklySavingsPlan(saved, target) {
  ...
  return plans.map(p => ({
    label: p.label,
    weekly: Math.ceil(remaining / (p.months * 4.33))
  }));
}
...
async function renderGoals() {
  ...
  let html = goals.map(g => {
    ...
    const plan = getWeeklySavingsPlan(g.saved, g.target);
    const weeklySavingsHtml = plan ? `
      <div class="weekly-savings-plan">
        <div class="weekly-savings-title">Weekly savings to achieve goal</div>
        <div class="weekly-savings-remaining">Remaining: ${fmtShort(remaining)}</div>
        <div class="weekly-savings-options">
          ${plan.map(p => `<div class="weekly-option">
            <span class="weekly-amount">${fmtShort(p.weekly)}</span>
            <span class="weekly-label">/week for ${p.label}</span>
          </div>`).join('')}
        </div>
      </div>` : `<div class="weekly-savings-plan weekly-savings-done">Goal achieved!</div>`;
...
async function completePulse() {
  ...
  const pulseResult = await api('/api/pulse', { method: 'POST', body: { ...pulseData, week } });
  if (!pulseResult) return;
  renderPulseSummary();
  renderDashboard();
}
...
async function toggleDarkMode() {
  ...
  const themeResult = await api('/api/user', { method: 'PUT', body: { theme: newTheme } });
  if (!themeResult) {
...
async function addExpense() {
  ...
  const expResult = await api('/api/transactions', {
    method: 'POST',
    body: {
      name: desc || 'Expense',
      category: cat,
...
async function addIncome() {
  ...
  const incResult = await api('/api/transactions', {
    method: 'POST',
    body: {
      name: desc || 'Income',
      category: 'income',
...
async function addToGoal() {
  ...
  const goalResult = await api(`/api/goals/${selectedGoalId}`, {
    method: 'PUT',
...
async function setBudget() {
  ...
  if (amount) {
    const budgetResult = await api('/api/user', { method: 'PUT', body: { budget: amount } });
...
async function addNewGoal() {
  ...
  const newGoalResult = await api('/api/goals', {
    method: 'POST',
    body: { name, emoji: selectedGoalEmoji, saved: 0, target }
  });
  if (!newGoalResult) return;
...
async function quickAddExpense() {
  ...
  const quickResult = await api('/api/transactions', {
    method: 'POST',
    body: {
...
async function addDue() {
  ...
  const dueResult = await api('/api/dues', {
    method: 'POST',
...
async function markDuePaid(dueId) {
  const payResult = await api(`/api/dues/${dueId}/pay`, { method: 'POST' });
  if (!payResult) return;
  await renderAll();
}
...
async function deleteDue(dueId) {
  if (confirm('Delete this payment?')) {
    const delResult = await api(`/api/dues/${dueId}`, { method: 'DELETE' });
    if (!delResult) return;
    await renderAll();
  }
}
...
async function renderEmiSection() {
  ...
  if (!allDuesCache) { allDuesCache = []; return; }
  renderEmiList(allDuesCache);
